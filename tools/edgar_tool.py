...
HEADERS={'User-Agent':config.SEC_USER_AGENT,'Accept-Encoding':'gzip, deflate','Host':'www.sec.gov'}
...
def _ciks():
    global _cik_map
    if _cik_map is None:
        r=_get(CIK_URL); _cik_map={v['ticker'].upper():str(v['cik_str']).zfill(10) for v in r.json().values()}
    return _cik_map
...
def _find(text,patterns):
    low=text.lower(); positions=[low.find(p.lower()) for p in patterns]; positions=[p for p in positions if p>=0]
...
def _extract_sections(text):
    ...
    specs={
      ...
      'financial_statements':(['item 8. financial statements','consolidated statements of','financial statements'],['item 9.','item 9a.'],26000),
    ...
    for key,(starts,ends,limit) in specs.items():
        s=_find(text,starts)
        if s>=0:
            e_candidates=[_find(text,ends)]
            e=[x for x in e_candidates if x>s]
            end=e[0] if e else min(len(text),s+limit)
            sections[key]=text[s:min(end,s+limit)]
    if not sections: sections['document_excerpt']=text[:config.MAX_SECTION_CHARS]
    return sections

def list_recent_filings(ticker, limit=12):
    ticker=ticker.upper(); cik=_ciks().get(ticker)
    if not cik:return {'error':f'{ticker} not found in SEC ticker map'}
    recent=_get(f'https://data.sec.gov/submissions/CIK{cik}.json').json()['filings']['recent']
    rows=[]
    for i,form in enumerate(recent['form']):
        if form in ('10-K','10-Q','8-K'):
            acc=recent['accessionNumber'][i]; acc_clean=acc.replace('-','')
            doc=recent['primaryDocument'][i]
            rows.append({'form':form,'filing_date':recent['filingDate'][i],'report_date':recent['reportDate'][i],'accession_number':acc,'primary_document':doc,'url':f"https://www.sec.gov/Archives/edgar/data/{int(cik)}/{acc_clean}/{doc}"})
...
def get_previous_filing(ticker, current_accession):
    ...
    for i,a in enumerate(recent['accessionNumber']):
        if a==current_accession: current_index=i; break
...
def check_new_filing(ticker,last_known_accession=None):
    ...
    Path(config.CACHE_DIR,f'{ticker}_meta.json').write_text(__import__('json').dumps({**latest,'filing_url':url,'cik':cik}),encoding='utf-8')
...
def get_cached_filing_text(ticker):
    p=Path(config.CACHE_DIR,f'{ticker.upper()}.txt'); return p.read_text(encoding='utf-8') if p.exists() else None

def get_filing_meta(ticker):
    p=Path(config.CACHE_DIR,f'{ticker.upper()}_meta.json')
    if not p.exists():return {}
    import json; return json.loads(p.read_text(encoding='utf-8'))
